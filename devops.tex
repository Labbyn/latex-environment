...
Praca przy projekcie zarządzana jest na systemie kontroli wersji \gls{Git} wraz z repozytorium na \gls{GitHub} i testowaniu rozwiązań przy pomocy \gls{GitHub Actions}.
\par
...
Dodatkowo \gls{Git} umożliwia nam podpięcie repozytorium do \gls{GitHub}a.
\par
\gls{GitHub} oferuje wiele opcji, pozwala na m.in. ograniczenie dostępu do repozytorium, ustawianie roli użytkowników, tworzenie \gls{Pull Request}ów, ustalanie reguł kiedy kod może zostać \gls{merge}'owany lub testowanie kodu za pośrednictwem \gls{GitHub Actions}.
\par
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/devops/struktura-projektu.png}
    \caption{Struktura projektu (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Przykładowa pierwsza wersja systemu przygotowana na prezentację wstępną (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Przykładowy \gls{Pull Request} (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Uruchamianie testów używając komentarza (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Przykładowe wywołanie testów \gls{API} (opracowanie własne)}
    \label{img:przykladowe-wywolanie-testow-api}
\end{figure}
\subsubsection{Budowanie kontenerów i wrzucanie ich na DockerHub}
\par
...
\begin{figure}[H]
    ...
    \caption{Przykładowy błąd z "lintera" (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Przykładowe zadanie z widocznym statusem implementacji (opracowanie własne)}
...
\subsubsection{Integrowanie zmian}
Podobnie jak przy repozytorium "Labbyn" zmiany integrowane są za pomocą \glslink{Pull Request}{PR}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/WIP.png}
    \caption{Konfiguracja \gls{Docker Compose} (opracowanie własne)}
    \label{img:wip_8}
\end{figure}
\par
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/devops/nakladka-na-glowny-plik-dockerowy.png}
    \caption{"Nakładka" na głowny plik \gls{Docker}a, który edytuje odpowiednie pola (opracowanie własne)}
...
\begin{figure}[H]
    ...
    \caption{Skrypt do zarządzania aplikacją (opracowanie własne)}
...
Spowodowane jest to tym, że przy budowie obrazu dockerowego posiłkuje się już stworzonymi warstwami, jeśli któraś warstwa (każda instrukcja to jedna warstwa) jest inna to reszta musi zostać ponownie zbudowana co wydłuża całkowity czas budowania obrazu.
\par
...
\begin{figure}[H]
    ...
    \caption{Dockerfile aplikacji "frontend" (opracowanie własne)}
...
Celem było ułątwienie testowania, jednocześnie nie obciążając nadmiernie platform deweloperskich członków zespołu.
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/devops/konfiguracja-dwoch-maszyn-wirtualnych.png}
    \caption{Konfiguracja dwóch wirtualnych maszyn (opracowanie własne)}
